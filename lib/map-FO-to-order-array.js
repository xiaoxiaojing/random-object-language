...
const {LoopbackError} = require('./error')

module.exports = function buildRelationArray (FO) {
  const FFArray = buildFFArray(FO)
  const [nonRelation, relation] = _.partition(FFArray, ([path, FF]) => FF.$type !== DEPENDANT_NODE)
...
function resolveDependantFF (path, FF) {
  ...
  while (_.size(FFStack)) {
    ...
    if (!_.isArray(dependsOn)) {
      dependantArray.push([path, dependsOn])
    } else {
    ...
    const possibleValue = _.filter(_.map(mapArray, fp.prop(2)).concat(defaultValue))
